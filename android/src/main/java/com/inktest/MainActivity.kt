...
class MainActivity : AppCompatActivity() {
    ...
    /**
     * Abgleich in beide Richtungen — erst senden, dann holen. Andersherum
     * überschriebe die Serverfassung die eigenen, noch nicht gesendeten Striche.
    ...
    private fun triggerSync() {
        ...
        Thread {
            try {
                ...
                val push = sync.pushDocument(document)
                prefs.lastSyncTime = System.currentTimeMillis()
                val pull = sync.pullDocument(document) { page -> repository.savePage(page) }
                repository.saveDocumentStructure(document)
                repository.flush()
                runOnUiThread {
                    currentSection = null
